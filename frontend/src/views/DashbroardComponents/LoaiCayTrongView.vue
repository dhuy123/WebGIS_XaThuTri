<template>
    <div class="container1">
        <div class="content-document">
            <div class="tile-document">
                <h1>THÔNG TIN LOẠI CÂY TRỒNG </h1>
            </div>
            <hr />
    ...
    <!-- model thêm mới  -->
    <div class="model-add">
        <a-modal v-model:open="openModel" title="Thêm mới loại cây trồng" width="600px" :footer="null">
...
import { getLoaiHienTrang, updateById, createLoaiHienTrang, deleteById, searchLoaiHienTrang1 } from '@/api/api_loai_hien_trang';
...
const columns = [
    ...
    {
        title: 'Mã hiện trạng',
        dataIndex: 'ma_hien_trang',
        width: '10%',
        align: 'center',
    },
    {
        title: 'Tên hiện trạng',
        dataIndex: 'ten_hien_trang'
    },
    {
        title: 'Mô tả',
        dataIndex: 'mo_ta',
...
const fetchData = async (page = current.value, limit = pageSize.value) => {
    try {
        loading.value = true;
        const response = await getLoaiHienTrang(page, limit);
...
const handleAdd = async (data) => {
    try {
        ...
        message.success('Thêm mới loại cây trồng thành công');
        openModel.value = false;
        await fetchData(current.value, pageSize.value);
    } catch (error) {
        console.error('Lỗi khi thêm mới loại cây trồng:', error);
    }
};
