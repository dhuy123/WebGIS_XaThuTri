...
import { getXepHangDiTich, updateById, createLoaiHienTrang, deleteById, searchXepHangDiTich } from '@/api/api_xep_hang_di_tich';
...
const columns = [
    ...
    {
        title: 'Mã xếp hạng',
        dataIndex: 'ma_xep_hang',
        width: '10%',
        align: 'center',
    },
    {
        title: 'Tên xếp hạng',
        dataIndex: 'ten_xep_hang'
    },
    {
        title: 'Mô tả',
        dataIndex: 'mo_ta',
...
const fetchData = async (page = current.value, limit = pageSize.value) => {
    try {
        loading.value = true;
        const response = await getXepHangDiTich(page, limit);
