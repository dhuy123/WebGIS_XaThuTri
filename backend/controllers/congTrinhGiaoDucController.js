...
const countCongTrinhGiaoDuc = async (req, res) => {
    try {
        const count = await congTrinhGiaoDucModel.countCongTrinhGiaoDuc();
        return res.status(200).json({ 
            message: 'Tổng số công trình giáo dục ',
            data: count 
        });
    } catch (error) {
        console.error('Lỗi khi đếm công trình giáo dục:', error);
        return res.status(500).json({ message: 'Lỗi khi đếm công trình giáo dục: ' + error.message });
    }
}
